...
class network():
    ...
    def set_dir(self, network_folder):
...
class fixed_network(network):
    def __init__ (self, network_folder, blocked_routes = [], node_file = 'nodes.nod.xml', edge_file = 'edges.edg.xml', network_output_file = 'network.net.xml', time_start = "0", time_end = "3600", route_file = 'route.xml', route_output_file = 'routes.rou.xml', sumo_cfg_file = 'sumo_network.sumocfg'):
        super().__init__()
        self.set_dir(network_folder)
        self.network_file_config(node_file, edge_file, network_output_file)
        self.route_file_config(network_output_file, blocked_routes, time_start, time_end, route_file, route_output_file, sumo_cfg_file)
        self.cd_parent()

        # set class variables
        self.network_file = os.path.join(network_folder, network_output_file)
        self.blocked_routes = blocked_routes
        

    def network_file_config(self, node_file, edge_file, network_output_file):
        """
    ...
    def route_file_config(self, network_output_file, blocked_routes, time_start, time_end, route_file, route_output_file, sumo_cfg_file):
        """
...
class osm_network(network):
    def __init__ (self, network_folder, osm_file, blocked_routes = [], network_output_file = 'osm_network.net.xml'): #, blocked_routes, time_start = "0", time_end = "3600", route_file = 'route.xml', route_output_file = 'routes.rou.xml', sumo_cfg_file = 'sumo_network.sumocfg'
        super().__init__()
        self.set_dir(network_folder)
        self.network_file_config(osm_file, network_output_file)
        # self.route_file_config(network_output_file, blocked_routes, time_start, time_end, route_file, route_output_file, sumo_cfg_file)
        self.cd_parent()
    ...
    def network_file_config(self, osm_file, network_output_file):
        """
        Configures the SUMO network files.
        ...
        def osm_SUMO_setup():
            # Convert OSM XML to SUMO format
            try:
                print(f"**Converting OSM file to SUMO format --> {osm_file} ...**")
                os.system(f'netconvert --osm-files={osm_file} --output-file={network_output_file} --geometry.remove --ramps.guess --junctions.join --tls.unset=all')   
                print("**Convert Completed!**")
                print("**Successfully Convert OSM to SUMO**\n")
            except Exception as e:
                print(f"**Error in converting OSM file to SUMO format: {e}**")
                sys.exit(1)
